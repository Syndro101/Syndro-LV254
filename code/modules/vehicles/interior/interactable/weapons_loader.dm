...
/*----------------------------
Skilless version for the jeeps
-----------------------------*/

/obj/structure/weapons_loader_skilless
	name = "simplified ammunition loader"
	desc = "A hefty piece of machinery that sorts, moves and loads various ammunition into the correct guns, this one doesnt require training."
	icon = 'icons/obj/vehicles/interiors/general.dmi'
	icon_state = "weapons_loader"
...
// Loading new magazines
/obj/structure/weapons_loader_skilless/attackby(obj/item/I, mob/user)
	if(!istype(I, /obj/item/ammo_magazine/hardpoint))
		return ..()
...
// Hardpoint reloading
/obj/structure/weapons_loader_skilless/attack_hand(mob/living/carbon/human/user)
	if(!user || !istype(user))
		return
	handle_reload(user)

/obj/structure/weapons_loader_skilless/proc/reload_ammo()
	set name = "Reload Ammo"
	set category = "Object"
	set src in range(1)
...
/obj/structure/weapons_loader_skilless/proc/handle_reload(mob/living/carbon/human/user)
	//something went bad, try to reconnect to vehicle if user is currently buckled in and connected to vehicle
	if(!vehicle)
		if(isVehicle(user.interactee))
			vehicle = user.interactee
